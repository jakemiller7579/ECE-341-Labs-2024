...
void clearLineLCD(unsigned int line) //line one or two? arguments 1 or 2
{
    if (line==1)
    {
        writeLCD(0,0x80);
...
void LCD_putc(char c)
{
    ...
    if(busyLCD()==0)//not busy
    {
        ...
        else if(c=='\r')//\r--carriage return: return to beginning of current line
        {
            if ((a>=0x00) & (a<=0x0f))
            {
                writeLCD(0,0x80);
